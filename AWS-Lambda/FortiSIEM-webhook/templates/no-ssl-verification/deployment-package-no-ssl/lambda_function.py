...
def lambda_handler(event, context):
    ...
    if result and webhookUrl is not None:
        webhookAuth = None
        ...
        try:
            r = requests.post(webhookUrl, 
                              headers={'Content-type': 'application/json', 
                                       'Authorization': 'Basic ' + os.getenv('AuthHeader')}, 
            ...
            logger.error('Webhook URL: %s', webhookUrl)
            logger.error('Request headers: %s', {
                'Content-type': 'application/json',
                'Authorization': 'Basic ' + os.getenv('AuthHeader')
            })
            logger.error('Request body: %s', json.dumps(data))
            httpResult = {
                'statusCode': 500,
                'body': 'Cannot connect to remote server ' + webhookUrl
            }
    else:
